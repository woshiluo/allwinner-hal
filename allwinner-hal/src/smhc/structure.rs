...
const MAX_DMA_DES_COUNT: usize = 2;
...
impl<'a, S: AsRef<RegisterBlock>, P> SdCard<'a, S, P> {
    ...
    pub fn read_block(&self, blocks: &mut [Block], start_block_idx: u32) {
        log::info!("read block {}", blocks.len());
        let length = blocks.len() as u32;
        if length == 0 {
            panic!("Invalid read block length = 0");
        }
        const MAX_SECTOR_PRE_DMA_DES: usize =
            IDMACDescriptor1::MAX_BUFFER_SIZE as usize / Block::LEN;
        loop {
            log::info!("qwq");
            ...
            for i in 0..blocks.len() {
                ...
                dma_desc[i].set_buffer_address(core::ptr::addr_of!(blocks[i].contents[0]) as _);
                dma_desc[i]
                    .set_next_descriptor_address(core::ptr::addr_of!(dma_desc[i + 1usize]) as _);
            }
            ...
            unsafe {
                asm!("fence");
                log::info!("qbq");
                smhc.global_control.modify(|val| val.set_dma_reset());
                while !smhc.global_control.read().is_dma_reset_cleared() {
                    core::hint::spin_loop();
                }
                smhc.dma_control.modify(|val| val.reset_dma());
                ...
                smhc.fifo_water_level.modify(|val| {
                    ...
                    val.set_burst_size(BurstSize::SixteenBit)
                        .set_receive_trigger_level(240)
                        .set_transmit_trigger_level(15)
                });
                smhc.dma_descriptor_base
                    .modify(|_| core::ptr::addr_of!(dma_desc[0]) as _);
                smhc.dma_control
                    .modify(|val| val.enable_dma().enable_fix_burst_size());
                let addr = smhc.dma_descriptor_base.read();
            ...
            let mut flag = true;
            for i in 0..blocks.len() {
                if dma_desc[i].des0.is_error_enable() || dma_desc[i].des0.is_hold_enable() {
                    flag = false;
                }
            }
            if flag {
